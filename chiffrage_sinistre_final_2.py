...
# CONSTANTES
# =========================================================
# Fichier CSV pré-nettoyé (généré par nettoyer_donnees.py)
CARBON_DATA_CSV = "carbon_data_clean.csv"
# Fichier HTML brut en secours si le CSV n'existe pas
CARBON_DATA_HTML = "carbon_data.html"
SELECTOR_MAP = {
...
# =========================================================
# CHARGEMENT DES DONNÉES — CSV nettoyé (prioritaire) ou HTML brut (secours)
# =========================================================
@st.cache_data
def load_df(csv_path: str = None) -> pd.DataFrame:
    """Charge les données carbone depuis le CSV pré-nettoyé.
    Si le CSV n'existe pas, tente de lire le fichier HTML brut en secours."""
    base = Path(__file__).parent
    csv_file = base / CARBON_DATA_CSV
    html_file = base / CARBON_DATA_HTML

    if csv_file.exists():
        df = pd.read_csv(csv_file, encoding="utf-8-sig")
        # S'assurer que les colonnes attendues sont présentes
        expected = ["Categorie", "Sous_categorie", "Produit_process",
                    "Unite", "Type_prestation", "Prestation", "Emissions_CO2"]
        for col in expected:
            if col not in df.columns:
                df[col] = ""
    elif html_file.exists():
        # Secours : lecture du HTML brut (peut contenir des artefacts RTF)
        tables = pd.read_html(str(html_file))
        if not tables:
        ...
        st.warning(
            "⚠️ Le fichier carbon_data_clean.csv est introuvable. "
            "L'application utilise le fichier HTML brut en secours. "
            "Exécutez `python nettoyer_donnees.py` pour générer le CSV nettoyé."
        )
    else:
        raise FileNotFoundError(
            f"Aucune source de données trouvée. Placez {CARBON_DATA_CSV} ou "
            f"{CARBON_DATA_HTML} dans le répertoire de l'application."
        )
    ...
    return df
# =========================================================
# CHARGEMENT DES DONNÉES — fichier alternatif utilisateur
# =========================================================
...
def apply_column_mapping(raw_df: pd.DataFrame, mapping: dict) -> pd.DataFrame:
    ...
    return df
# =========================================================
# CHARGEMENT DES DONNÉES — annuaire entreprises (Excel)
# =========================================================
@st.cache_data
...
def _save_companies(df: pd.DataFrame):
    """Écrit le DataFrame des entreprises dans le fichier Excel et vide le cache."""
...
def get_reduction_color(pct: float) -> str:
    ...
    return "#e65100"
# =========================================================
# CALLBACKS — exclusion mutuelle entre colonnes Standard / Bas carbone
# =========================================================
...
# =========================================================
# BARRE LATÉRALE : NAVIGATEUR D'ENTREPRISES
# =========================================================
...
# =========================================================
# AFFICHAGE : RECHERCHE PAR MOT-CLÉ
# =========================================================
def _make_search_entry(row: pd.Series, qty: float, price=None):
    """Construit une entrée de panier à partir d'une ligne de résultat de recherche."""
...
def render_search(df: pd.DataFrame):
    ...
    query = st.text_input(
        ...
        key="search_query",
        placeholder="ex. laine chanvre plomberie (les accents sont optionnels)",
        help="La recherche ignore les accents : taper « fenetre » trouvera « fenêtre ».",
    )
    ...
    # Dédupliquer : un seul résultat par Produit_process
    unique_products = (
    ...
    # ---- Ajout au panier depuis les résultats de recherche ----
    st.markdown("---")
    ...
    # Étape 2 : afficher les prestations disponibles pour ce produit
    ...
    # Récupérer la ligne correspondant au produit et à la prestation sélectionnés
    ...
    with btn_s2:
        if st.button("➕ Ajouter à **Configuration 2**", key="search_add_c2",
            ...
            st.rerun()
# =========================================================
# AFFICHAGE : TABLEAUX DE DONNÉES COMPLETS
# =========================================================
...
# =========================================================
# AFFICHAGE : NAVIGATION PAR CATÉGORIE
# =========================================================
...
# =========================================================
# AFFICHAGE : SÉLECTION PRODUIT EN DEUX COLONNES
# =========================================================
def render_product_selection(candidates: pd.DataFrame, key_prefix: str):
    ...
    if std_choice != NONE_SENTINEL:
        ...
        return None
# =========================================================
# AFFICHAGE : PANNEAU DE SÉLECTION
# =========================================================
...
def _render_basket(config_key: str):
    ...
    if not basket:
        ...
        return
    # Construire un DataFrame éditable avec les colonnes modifiables
    edit_rows = []
    ...
    # Détecter si l'utilisateur a modifié une valeur d'émissions
    ...
    if st.button("💾 Appliquer les modifications", key=f"apply_edit_{config_key}",
        ...
        for i in range(len(edited)):
            ...
            # Récupérer l'entrée originale pour les champs non modifiables
            if i < len(basket):
                orig = basket[i].copy()
            else:
                # Nouvelle ligne ajoutée via data_editor — créer une entrée minimale
                orig = {
    ...
    # ---- Totaux calculés (résumé en lecture seule) ----
...
# =========================================================
# AFFICHAGE : COMPARAISON (graphiques barres empilées + camemberts)
# =========================================================
def render_comparison():
    ...
    # Utiliser Categorie_old (les 13 catégories d'origine) pour la ventilation
    ...
    if all_cats:
        ...
        # Construire un graphique à barres empilées par configuration
        fig_bar = go.Figure()
    ...
    for col_w, df_x, lbl in [(d1, df1, "Configuration 1"), (d2, df2, "Configuration 2")]:
        with col_w:
            ...
            if not df_x.empty:
                ...
                st.info("Vide")
# =========================================================
# AFFICHAGE : SOURCE DE DONNÉES ALTERNATIVE
# =========================================================
def render_data_source_settings():
    st.subheader("Source de données")
    st.markdown(
        "Par défaut, l'application utilise **carbon_data_clean.csv**, un fichier extrait et néttoyé de la base de données dans Avensis au format .html. "
...
# =========================================================
# AFFICHAGE : FORMULAIRE D'AJOUT D'ENTREPRISE
# =========================================================
...
# =========================================================
# AFFICHAGE : MODIFIER / SUPPRIMER DES ENTREPRISES
# =========================================================
def render_manage_companies():
    """Éditeur tabulaire pour modifier ou supprimer des lignes d'entreprises."""
    ...
    # Afficher le tableau complet dans un éditeur de données
    ...
    # Suppression rapide par numéro de ligne
...
def render_documentation():
    """Affiche un bouton de téléchargement pour la documentation PDF."""
...
def main():
    ...
    inject_theme()
    render_logo()
    # Charger les données carbone par défaut
    default_df = load_df()
    ...
    # Charger les données entreprises
    companies_df = load_companies()
    # Barre latérale : navigateur d'entreprises
    ...
    # Jeux de données complets en bas de page
    st.divider()
